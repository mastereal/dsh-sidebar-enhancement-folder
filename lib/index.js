// dsh-sidebar-enhancement-folder — Host half
// Single route: reveal an absolute path in the OS file manager.
//   GET /dsh-sidebar-enhancement-folder/reveal?path=<absolute>
import { existsSync, statSync } from "node:fs";
import { dirname, resolve, isAbsolute, basename } from "node:path";
import { spawn } from "node:child_process";

export const name = "dsh-sidebar-enhancement-folder";
...
function sessionCwd(ctx, sessionId) {
  ...
  if (sessionId) {
    console.log(`[dsh-sidebar-enhancement-folder] sessionCwd failed for ${sessionId} (falling back to process.cwd())`);
  }
  return "";
}
...
function reveal(ctx, rawPath, sessionId, res) {
  ...
  try {
    ...
    child.unref();
    activateExplorerWindow(open);
    console.log(`[dsh-sidebar-enhancement-folder] reveal ${select ? "select" : "open"} ${open}`);
...
export function apply(ctx) {
  ...
  return webServer.register({
    kind: "prefix",
    path: "/dsh-sidebar-enhancement-folder",
    handler: (req, res) => {
      if (!trusted(req)) {
        res.writeHead(403);
        res.end("forbidden");
      ...
      try {
        ...
        if (url.pathname !== "/dsh-sidebar-enhancement-folder/reveal") {
          res.writeHead(404);
          res.end("not found");
          return;
        }
...
// sessionCwd() always returned "" — relative reveal paths resolved against
// process.cwd() (the DSH service's launch directory) instead of the session
// workspace.
